...
//   abstract makeSound(): void;  // Abstract method (must be implemented in subclasses)
...
const studentAbove80: { name: string; score: number } | undefined =
  students.find((student: { name: string; score: number }) => {
...
const ages = [12, 25, 17, 19, 30, 16, 22];

// Use filter() to get adults
const adults = ages.filter((val)=>{
    return val>=18;
})
...
const total: number= numbers.reduce((val1 : number,val2 : number)=>{
   return val1+val2;
})
...
const fahrenheitTemperatures = celsiusTemperatures.map((val)=>{
    let farTemperature= (val*(9/5))+32;
...
const updatedSalaries = employees.map((emp : {name : string, salary: number})=>{
